...
{/* Latest Ruling Card - Landing Page Style */}
{(()=>{const latest=active.analyses[active.analyses.length-1];const isAllow=latest.ruling==="Allow";const isDeny=latest.ruling==="Deny";const bgColor=isAllow?"rgba(40,167,69,.03)":isDeny?"rgba(229,57,53,.03)":"rgba(245,124,0,.03)";const borderColor=isAllow?"rgba(40,167,69,.12)":isDeny?"rgba(229,57,53,.12)":"rgba(245,124,0,.12)";const textColor=isAllow?"#1B7D36":isDeny?"#C62828":"#E65100";return(
<div style={{padding:"20px",background:"#fff",borderRadius:16,border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",marginBottom:12}}>
<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><span style={{width:10,height:10,borderRadius:"50%",background:isAllow?"var(--green)":isDeny?"var(--red)":"var(--orange)"}}></span><span style={{fontSize:14,fontWeight:700,color:"var(--g800)"}}>{active.claimNumber}</span><span style={{padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:600,color:textColor,background:bgColor,border:`1px solid ${borderColor}`}}>{stageOf(active.stage).label}</span></div>
<div style={{padding:"12px 16px",background:bgColor,borderRadius:12,border:`1px solid ${borderColor}`,marginBottom:12}}><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{width:8,height:8,borderRadius:"50%",background:isAllow?"var(--green)":isDeny?"var(--red)":"var(--orange)"}}></span><span style={{fontSize:15,fontWeight:700,color:textColor}}>RULING PREDICTION: {latest.ruling}</span></div></div>
<div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:12}}>
{["Active employer account","Worker performing duties","Injury by accident/disease","Arose from employment","Resulting disability"].map((check,i)=>{const pass=isAllow||(!isDeny&&i<3);return(
<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:pass?"rgba(40,167,69,.03)":"rgba(229,57,53,.03)",borderRadius:10,border:`1px solid ${pass?"rgba(40,167,69,.08)":"rgba(229,57,53,.08)"}`}}>
<div style={{width:20,height:20,borderRadius:"50%",background:pass?"var(--green)":"var(--red)",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,flexShrink:0}}>{pass?"✓":"✗"}</div>
...
{[{label:"New Case",action:()=>{setModal("new");setCmdOpen(false)}},{label:"Ask Advisor",action:()=>{setActive(null);setMsgs([]);nav("chat");setCmdOpen(false)}},{label:"Quick Note",action:()=>{setQuickNote({open:true,text:"",caseId:""});setCmdOpen(false)}}].map((a,i)=>(
