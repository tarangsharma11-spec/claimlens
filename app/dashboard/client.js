...
<div style={{display:"flex",gap:6,marginBottom:4,flexWrap:"wrap"}}><button onClick={()=>openChat(active)} style={{padding:"7px 16px",borderRadius:980,fontSize:13,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer"}}>Open Advisor</button><button onClick={()=>{const w=window.open("","_blank");w.document.write("<html><head><title>"+active.claimNumber+"</title><style>body{font-family:sans-serif;max-width:700px;margin:40px auto;color:#333}h1{font-size:24px}h2{font-size:16px;color:#666;border-bottom:1px solid #ddd;padding-bottom:4px;margin-top:20px}.meta{color:#888;font-size:13px}.section{margin:12px 0}.item{padding:4px 0;font-size:14px}</style></head><body>");w.document.write("<h1>"+active.claimNumber+"</h1>");w.document.write("<div class=meta>"+active.worker+" | "+active.employer+" | "+fmt(active.injuryDate)+" | "+active.injuryType+"</div>");w.document.write("<div class=meta>Stage: "+stageOf(active.stage).label+" | Created: "+fmt(active.createdAt)+"</div>");w.document.write("<h2>Summary</h2><p>"+getCaseSummary(active)+"</p>");if(active.description)w.document.write("<h2>Description</h2><p>"+active.description+"</p>");if(active.analyses?.length)w.document.write("<h2>Analyses</h2>"+active.analyses.map(a=>"<div class=item><strong>"+a.ruling+"</strong> - "+fmt(a.date)+"</div>").join(""));if(active.documents?.length)w.document.write("<h2>Documents ("+active.documents.length+")</h2>"+active.documents.map(d=>"<div class=item>"+d.name+" ["+d.tag+"]</div>").join(""));if(active.providers?.length)w.document.write("<h2>Providers</h2>"+active.providers.map(p=>"<div class=item>"+p.name+" ("+p.type+") - Records: "+p.status+"</div>").join(""));if(active.tasks?.length)w.document.write("<h2>Tasks</h2>"+active.tasks.map(t=>"<div class=item>"+(t.done?"[x] ":"[ ] ")+t.text+"</div>").join(""));w.document.write("<h2>Timeline</h2>"+(active.timeline||[]).map(e=>"<div class=item>"+fmt(e.date)+" - "+e.note+"</div>").join(""));w.document.write("</body></html>");w.document.close();w.print()}} style={{padding:"7px 12px",borderRadius:980,fontSize:13,fontWeight:500,border:"1px solid var(--g200)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Print</button><button onClick={()=>setModal("stage")} style={{padding:"7px 12px",borderRadius:980,fontSize:13,fontWeight:600,border:".5px solid var(--g300)",background:"transparent",color:"var(--g600)",cursor:"pointer"}}>Change Status</button><button onClick={()=>{const cl={...active};cl.pinned=!cl.pinned;saveClaim(cl)}} style={{padding:"7px 12px",borderRadius:980,fontSize:13,fontWeight:500,border:"1px solid var(--g200)",background:active.pinned?"rgba(0,113,227,.06)":"#fff",color:active.pinned?"var(--blue)":"var(--g600)",cursor:"pointer"}}>{active.pinned?"Unpin":"Pin"}</button><button onClick={()=>{const cl={...active};const dup={...cl,id:Date.now().toString(36)+Math.random().toString(36).slice(2,6),claimNumber:"CL-"+Date.now().toString(36).slice(-6).toUpperCase(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),analyses:[],messages:[],timeline:[{date:new Date().toISOString(),type:"created",note:"Duplicated from "+active.claimNumber}],stage:"new"};setClaims(prev=>{const next=[dup,...prev];persistClaims(user.email,next);return next});alert("Case duplicated as "+dup.claimNumber)}} style={{padding:"7px 12px",borderRadius:980,fontSize:13,fontWeight:500,border:"1px solid var(--g200)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Duplicate</button><button onClick={()=>{if(confirm("Delete this case permanently? (To archive instead, change status to Closed)"))delClaim(active.id)}} style={{padding:"7px 12px",borderRadius:980,fontSize:13,fontWeight:600,border:".5px solid var(--red-border)",background:"transparent",color:"var(--red)",cursor:"pointer"}}>Delete</button><button onClick={()=>{if(timer.running&&timer.caseId===active.id){const elapsed=Math.floor((Date.now()-timer.start)/60000);setClaims(prev=>{const next=prev.map(cc=>{if(cc.id===active.id){return{...cc,timeEntries:[...(cc.timeEntries||[]),{date:new Date().toISOString(),minutes:elapsed,desc:"Timed session"}],timeline:[...(cc.timeline||[]),{date:new Date().toISOString(),type:"note",note:"Time logged: "+elapsed+" minutes"}]}}return cc});persistClaims(user.email,next);if(active)setActive(next.find(x=>x.id===active.id));return next});setTimer({running:false,caseId:null,start:null,entries:[]})}else{setTimer({running:true,caseId:active.id,start:Date.now(),entries:[]})}}} style={{padding:"7px 12px",borderRadius:980,fontSize:13,fontWeight:500,border:"1px solid var(--g200)",background:timer.running&&timer.caseId===active.id?"var(--red-light)":"#fff",color:timer.running&&timer.caseId===active.id?"var(--red)":"var(--g600)",cursor:"pointer"}}>{timer.running&&timer.caseId===active.id?"Stop Timer":"Start Timer"}</button></div>
