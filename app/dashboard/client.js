...
<span style={{fontSize:10,fontWeight:700,color:dt.color}}>{dt.id.slice(0,2).toUpperCase()}</span>
</div>
...
<button onClick={()=>{if(confirm("Delete document: "+d.name+"?")){const cl={...active};cl.documents=cl.documents.filter((_,j)=>j!==i);cl.timeline=[...(cl.timeline||[]),{date:new Date().toISOString(),type:"document",note:"Deleted: "+d.name}];saveClaim(cl)}}} style={{padding:"3px 8px",borderRadius:6,fontSize:10,fontWeight:500,border:"1px solid var(--red-border)",background:"var(--red-light)",color:"var(--red)",cursor:"pointer"}}>Delete</button>
</div>
</div>)})}
</div>
</>}
