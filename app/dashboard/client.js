...
{/* NAV */}
<nav style={{height:56,padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"var(--white)",borderBottom:"1px solid var(--card-border)",flexShrink:0,zIndex:100}}>
  <div style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",flexShrink:0}} onClick={()=>nav("home")}><div style={{width:28,height:28,borderRadius:7,background:"var(--g900)",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="#fff" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/></svg></div><span style={{fontSize:15,fontWeight:700,letterSpacing:-.3}}>CaseAssist</span></div>
  <div style={{display:"flex",gap:1,background:"var(--g100)",borderRadius:12,padding:2}} className="desktop-nav">{[{id:"home",label:"Home"},{id:"claims",label:"My Cases"},{id:"advisor",label:"Advisor"},{id:"templates",label:"Templates"},{id:"analytics",label:"Analytics"},{id:"glossary",label:"Glossary"}].map(t=>(<button key={t.id} onClick={()=>{if(t.id==="advisor"){setActive(null);setMsgs([]);nav("chat")}else if(t.id==="templates")setShowTemplates(true);else if(t.id==="analytics")setShowAnalytics(true);else if(t.id==="glossary")setShowGlossary(true);else nav(t.id)}} style={{padding:"6px 14px",borderRadius:8,fontSize:12.5,fontWeight:600,color:(view===t.id||(t.id==="advisor"&&view==="chat"&&!active)||(t.id==="templates"&&showTemplates)||(t.id==="analytics"&&showAnalytics)||(t.id==="glossary"&&showGlossary))?"var(--g900)":"var(--g500)",background:(view===t.id||(t.id==="advisor"&&view==="chat"&&!active)||(t.id==="templates"&&showTemplates)||(t.id==="analytics"&&showAnalytics)||(t.id==="glossary"&&showGlossary))?"#fff":"transparent",border:"none",cursor:"pointer",boxShadow:(view===t.id||(t.id==="advisor"&&view==="chat"&&!active))?"0 1px 3px rgba(0,0,0,.04)":"none",letterSpacing:-.1}}>{t.label}</button>))}</div>
  <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}><button onClick={()=>setShowNotifs(!showNotifs)} style={{width:34,height:34,borderRadius:12,border:"1px solid #E8EAEF",background:showNotifs?"var(--g900)":"#fff",color:showNotifs?"#fff":"var(--g500)",cursor:"pointer",position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0"/></svg>{(()=>{const u=getNotifications(claims).filter(x=>x.type==="urgent").length;return u>0?<span style={{position:"absolute",top:-3,right:-3,width:17,height:17,borderRadius:"50%",background:"#E53935",color:"#fff",fontSize:9,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>{u}</span>:null})()}</button><button onClick={()=>setQuickNote({open:true,text:"",caseId:""})} style={{width:34,height:34,borderRadius:12,border:"1px solid #E8EAEF",background:"#fff",color:"var(--g500)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} title="Quick Note (any case)"><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/></svg></button><button onClick={()=>setModal("new")} style={{padding:"7px 18px",borderRadius:12,fontSize:12.5,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer"}}>+ New Case</button><button onClick={()=>signOut({callbackUrl:"/login"})} style={{padding:"7px 12px",borderRadius:12,fontSize:12,fontWeight:500,border:"1px solid #E8EAEF",background:"#fff",color:"var(--g500)",cursor:"pointer"}}>Sign Out</button></div>
</nav>
...
<div style={{padding:"20px 24px",background:"linear-gradient(135deg, #0062CC 0%, #0088F0 40%, #00B4D8 70%, #2EC4B6 100%)",borderRadius:20,marginBottom:20,color:"#fff",position:"relative",overflow:"hidden",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
  ...
  <div style={{display:"flex",gap:8,flexWrap:"wrap",position:"relative",zIndex:1}}>
    <button onClick={()=>setModal("new")} style={{padding:"9px 20px",borderRadius:100,fontSize:13,fontWeight:600,background:"rgba(255,255,255,.2)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,.25)",color:"#fff",cursor:"pointer"}}>+ New Case</button>
    <button onClick={()=>{setActive(null);setMsgs([]);nav("chat")}} style={{padding:"9px 20px",borderRadius:100,fontSize:13,fontWeight:600,background:"rgba(255,255,255,.2)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,.25)",color:"#fff",cursor:"pointer"}}>Ask Advisor</button>
  </div>
</div>
...
if(overdue.length===0&&upcoming.length===0&&openTasks.length===0)return null;
return(<div style={{padding:"14px 16px",background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",marginBottom:14}}>
...
{overdue.length>0&&<div style={{marginBottom:8}}>{overdue.slice(0,3).map((d,i)=><div key={i} onClick={()=>{const cc=claims.find(x=>x.id===d.caseId);if(cc)openClaim(cc)}} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",cursor:"pointer"}}><div style={{width:6,height:6,borderRadius:"50%",background:"var(--red)",flexShrink:0}}/><span style={{fontSize:12,color:"var(--red)",fontWeight:600}}>{d.claimNumber}</span><span style={{fontSize:12,color:"var(--g600)"}}>{d.label}</span><span style={{fontSize:10,color:"var(--red)",fontWeight:600,marginLeft:"auto"}}>{Math.abs(d.daysLeft)}d overdue</span></div>)}</div>}
{upcoming.length>0&&<div style={{marginBottom:8}}>{upcoming.slice(0,3).map((d,i)=><div key={i} onClick={()=>{const cc=claims.find(x=>x.id===d.caseId);if(cc)openClaim(cc)}} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",cursor:"pointer"}}><div style={{width:6,height:6,borderRadius:"50%",background:"var(--orange)",flexShrink:0}}/><span style={{fontSize:12,color:"var(--g700)",fontWeight:600}}>{d.claimNumber}</span><span style={{fontSize:12,color:"var(--g600)"}}>{d.label}</span><span style={{fontSize:10,color:"var(--orange)",fontWeight:600,marginLeft:"auto"}}>{d.daysLeft}d</span></div>)}</div>}
{openTasks.length>0&&<div>{openTasks.slice(0,3).map((t,i)=><div key={i} onClick={()=>{const cc=claims.find(x=>x.id===t.caseId);if(cc){openClaim(cc);setDetailTab("tasks")}}} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",cursor:"pointer"}}><div style={{width:14,height:14,borderRadius:4,border:"2px solid var(--g300)",flexShrink:0}}/><span style={{fontSize:12,color:"var(--g700)",fontWeight:600}}>{t.claimNumber}</span><span style={{fontSize:12,color:"var(--g600)"}}>{t.text}</span></div>)}</div>}
</div>)})()}

{/* STATS + TOOLS ROW */}
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(100px, 1fr))",gap:10,marginBottom:14}}>
  <div style={{padding:"14px",background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",textAlign:"center"}}><div style={{fontSize:26,fontWeight:800,letterSpacing:-1.5}}>{claims.length}</div><div style={{fontSize:10,fontWeight:500,color:"var(--g400)",marginTop:2}}>Total</div></div>
  {stageCounts.map(s=><div key={s.id} style={{padding:"14px",background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",textAlign:"center"}}><div style={{fontSize:26,fontWeight:800,letterSpacing:-1.5,color:s.color}}>{s.count}</div><div style={{fontSize:10,fontWeight:500,color:"var(--g400)",marginTop:2}}>{s.label}</div></div>)}
  
</div>

{/* TWO COLUMN LAYOUT */}
<div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:16,marginBottom:14}}>
  <div>
    {needsAttention.length>0&&<><div style={{fontSize:11,fontWeight:700,color:"var(--g500)",textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Needs Attention</div>{needsAttention.slice(0,4).map(c=><CaseSummaryCard key={c.id} claim={c} onClick={()=>openClaim(c)}/>)}</>}
  ...
  <div style={{overflow:"hidden",minWidth:0}}>
    <div style={{fontSize:11,fontWeight:700,color:"var(--g500)",textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Quick Tools</div>
    {[{label:"Case Templates",desc:"Start from a template",onClick:()=>setShowTemplates(true)},{label:"Benefit Calculator",desc:"Estimate LOE & NEL",onClick:()=>setShowCalc(true)},{label:"Analytics Dashboard",desc:"Portfolio insights",onClick:()=>setShowAnalytics(true)},{label:"Conflict Check",desc:"Search all cases for conflicts",onClick:()=>{const name=prompt("Enter a name to check for conflicts (worker, employer, or provider):");if(name){const q=name.toLowerCase();const matches=claims.filter(cl=>[cl.worker,cl.employer,...(cl.providers||[]).map(p=>p.name)].some(n=>(n||"").toLowerCase().includes(q)));if(matches.length>0)alert("Potential conflicts found in "+matches.length+" case(s): "+matches.map(m=>m.claimNumber+" ("+m.worker+" vs "+m.employer+")").join(", "));else alert("No conflicts found for: "+name)}}}].map((t,i)=>(
      <button key={i} onClick={t.onClick} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",cursor:"pointer",textAlign:"left",width:"100%",marginBottom:6}}>
        <div><div style={{fontSize:13,fontWeight:600,color:"var(--g900)"}}>{t.label}</div><div style={{fontSize:11,color:"var(--g500)"}}>{t.desc}</div></div>
      </button>
    ))}
    <div style={{fontSize:11,fontWeight:700,color:"var(--g500)",textTransform:"uppercase",letterSpacing:1,marginBottom:10,marginTop:16}}>Ask the Advisor</div>
    {scenarios.map((s,i)=><button key={i} onClick={()=>{setActive(null);setMsgs([]);nav("chat");setTimeout(()=>send(s.t),100)}} style={{padding:"12px 14px",borderRadius:12,background:"#fff",border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",cursor:"pointer",textAlign:"left",width:"100%",marginBottom:4,overflow:"hidden",maxWidth:"100%"}}><div style={{fontSize:13,fontWeight:600,marginBottom:1}}>{s.l}</div><div style={{fontSize:11,color:"var(--g500)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.t}</div></button>)}
  </div>
</div>
</>}
...
{/* Search bar */}
<div style={{marginBottom:12}}><input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="Search cases... (Cmd+K for quick switch)…" style={{width:"100%",padding:"10px 14px",borderRadius:12,border:"1px solid var(--card-border)",fontSize:13,outline:"none",background:"#fff",fontFamily:"inherit"}} /></div>
...
</>}</div>:displayed.map(c=><CaseSummaryCard key={c.id} claim={c} onClick={()=>openClaim(c)}/>)}
</div></div>)}
...
<div style={{display:"flex",gap:6,marginBottom:4,flexWrap:"wrap"}}><button onClick={()=>openChat(active)} style={{padding:"7px 16px",borderRadius:100,fontSize:13,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer"}}>Open Advisor</button><button onClick={()=>{const w=window.open("","_blank");const css="*{margin:0;padding:0;box-sizing:border-box}body{font-family:Helvetica,Arial,sans-serif;max-width:700px;margin:0 auto;padding:40px 32px;color:#1D1D1F;font-size:13px;line-height:1.5}h1{font-size:22px;font-weight:800;letter-spacing:-0.5px;margin-bottom:4px}h2{font-size:14px;font-weight:700;color:#86868B;text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid #E8E8ED;padding-bottom:6px;margin:24px 0 10px}.meta{color:#6E6E73;font-size:12px;margin-bottom:16px}.badge{display:inline-block;padding:2px 10px;border-radius:100px;font-size:11px;font-weight:600;margin-right:6px}.item{padding:6px 0;border-bottom:1px solid #F5F5F7}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0}.card{padding:10px 14px;border:1px solid #E8E8ED;border-radius:8px}.label{font-size:10px;text-transform:uppercase;letter-spacing:0.5px;color:#86868B;font-weight:600}.val{font-size:16px;font-weight:700;margin-top:2px}.footer{margin-top:32px;padding-top:12px;border-top:1px solid #E8E8ED;font-size:10px;color:#AEAEB2;text-align:center}@media print{body{padding:20px}}";w.document.write("<!DOCTYPE html><html><head><title>"+active.claimNumber+" - CaseAssist Report</title><style>"+css+"</style></head><body>");w.document.write("<div style=\"display:flex;align-items:center;gap:10px;margin-bottom:20px\"><div style=\"width:32px;height:32px;border-radius:8px;background:#1D1D1F;display:flex;align-items:center;justify-content:center\"><span style=\"color:#fff;font-size:14px;font-weight:800\">CA</span></div><div><div style=\"font-size:11px;color:#86868B;font-weight:600\">CASEASSIST REPORT</div><div style=\"font-size:10px;color:#AEAEB2\">Generated "+new Date().toLocaleDateString("en-CA",{year:"numeric",month:"long",day:"numeric"})+"</div></div></div>");w.document.write("<h1>"+active.claimNumber+"</h1>");w.document.write("<div class=meta>"+active.worker+" | "+active.employer+" | "+fmt(active.injuryDate)+" | "+active.injuryType+" | Stage: "+stageOf(active.stage).label+"</div>");const lr=active.analyses?.[active.analyses.length-1]?.ruling;if(lr)w.document.write("<div style=\"padding:10px 16px;border-radius:8px;margin-bottom:16px;font-weight:700;background:"+(lr==="Allow"?"#E8F8EC;color:#1B7D36":lr==="Deny"?"#FFF0EF;color:#CC2D26":"#FFF8EB;color:#A66A00")+"\">AI Ruling: "+lr+"</div>");w.document.write("<h2>Summary</h2><p>"+getCaseSummary(active)+"</p>");if(active.description)w.document.write("<h2>Description</h2><p>"+active.description+"</p>");if(active.analyses?.length){w.document.write("<h2>Analysis History</h2>");active.analyses.forEach(a=>{w.document.write("<div class=item><strong style=\"color:"+(a.ruling==="Allow"?"#34C759":a.ruling==="Deny"?"#FF3B30":"#FF9500")+"\">"+a.ruling+"</strong> - "+fmt(a.date)+"</div>")})};if(active.documents?.length){w.document.write("<h2>Documents ("+active.documents.length+")</h2>");active.documents.forEach(d=>{w.document.write("<div class=item>"+d.name+" <span class=badge style=\"background:#F0F1F3;color:#6E6E73\">"+d.tag+"</span></div>")})};if(active.providers?.length){w.document.write("<h2>Medical Providers</h2>");active.providers.forEach(p=>{w.document.write("<div class=item><strong>"+p.name+"</strong> ("+p.type+") - Records: <span style=\"color:"+(p.status==="received"?"#34C759":"#FF9500")+"\">"+p.status+"</span></div>")})};const v=active.valuation||{};const total=Object.values(v).reduce((s,x)=>s+(parseFloat(x)||0),0);if(total>0){w.document.write("<h2>Claim Valuation</h2><div class=grid>");VALUATION_FIELDS.forEach(f=>{if(v[f.id])w.document.write("<div class=card><div class=label>"+f.label+"</div><div class=val>$"+(parseFloat(v[f.id])||0).toLocaleString()+"</div></div>")});w.document.write("</div><div style=\"padding:12px 16px;background:#F0F7FF;border:1px solid #CCE0FF;border-radius:8px;margin-top:8px\"><div class=label>Total Claim Value</div><div style=\"font-size:22px;font-weight:800;color:#0071E3\">$"+total.toLocaleString()+"</div></div>")};if(active.tasks?.length){w.document.write("<h2>Tasks</h2>");active.tasks.forEach(t=>{w.document.write("<div class=item>"+(t.done?"<span style=\"color:#34C759\">Done</span> ":"<span style=\"color:#FF9500\">Open</span> ")+t.text+"</div>")})};w.document.write("<h2>Timeline</h2>");(active.timeline||[]).slice().reverse().forEach(e=>{w.document.write("<div class=item><span style=\"color:#86868B\">"+fmt(e.date)+"</span> "+e.note+"</div>")});w.document.write("<div class=footer>Generated by CaseAssist | caseassist.ca | Advisory tool only. Final decisions by authorized WSIB adjudicators.</div>");w.document.write("</body></html>");w.document.close();setTimeout(()=>w.print(),300)}} style={{padding:"7px 12px",borderRadius:100,fontSize:13,fontWeight:500,border:"1px solid var(--card-border)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Export PDF</button><button onClick={()=>setModal("stage")} style={{padding:"7px 12px",borderRadius:100,fontSize:13,fontWeight:600,border:"1px solid var(--g300)",background:"transparent",color:"var(--g600)",cursor:"pointer"}}>Change Status</button><button onClick={()=>{const cl={...active};cl.pinned=!cl.pinned;saveClaim(cl)}} style={{padding:"7px 12px",borderRadius:100,fontSize:13,fontWeight:500,border:"1px solid var(--card-border)",background:active.pinned?"rgba(0,113,227,.06)":"#fff",color:active.pinned?"var(--blue)":"var(--g600)",cursor:"pointer"}}>{active.pinned?"Unpin":"Pin"}</button><button onClick={()=>{const cl={...active};const dup={...cl,id:Date.now().toString(36)+Math.random().toString(36).slice(2,6),claimNumber:"CL-"+Date.now().toString(36).slice(-6).toUpperCase(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),analyses:[],messages:[],timeline:[{date:new Date().toISOString(),type:"created",note:"Duplicated from "+active.claimNumber}],stage:"new"};setClaims(prev=>{const next=[dup,...prev];persistClaims(user.email,next);return next});alert("Case duplicated as "+dup.claimNumber)}} style={{padding:"7px 12px",borderRadius:100,fontSize:13,fontWeight:500,border:"1px solid var(--card-border)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Duplicate</button><button onClick={()=>{if(confirm("Delete this case permanently? (To archive instead, change status to Closed)"))delClaim(active.id)}} style={{padding:"7px 12px",borderRadius:100,fontSize:13,fontWeight:600,border:"1px solid var(--red-border)",background:"transparent",color:"var(--red)",cursor:"pointer"}}>Delete</button><button onClick={()=>{if(timer.running&&timer.caseId===active.id){const elapsed=Math.floor((Date.now()-timer.start)/60000);setClaims(prev=>{const next=prev.map(cc=>{if(cc.id===active.id){return{...cc,timeEntries:[...(cc.timeEntries||[]),{date:new Date().toISOString(),minutes:elapsed,desc:"Timed session"}],timeline:[...(cc.timeline||[]),{date:new Date().toISOString(),type:"note",note:"Time logged: "+elapsed+" minutes"}]}}return cc});persistClaims(user.email,next);if(active)setActive(next.find(x=>x.id===active.id));return next});setTimer({running:false,caseId:null,start:null,entries:[]})}else{setTimer({running:true,caseId:active.id,start:Date.now(),entries:[]})}}} style={{padding:"7px 12px",borderRadius:100,fontSize:13,fontWeight:500,border:"1px solid var(--card-border)",background:timer.running&&timer.caseId===active.id?"var(--red-light)":"#fff",color:timer.running&&timer.caseId===active.id?"var(--red)":"var(--g600)",cursor:"pointer"}}>{timer.running&&timer.caseId===active.id?"Stop Timer":"Start Timer"}</button></div>
<StageProgress stage={active.stage}/>
{/* Stage guidance banner */}
<div style={{padding:"12px 16px",background:"rgba(0,113,227,.04)",border:"1px solid rgba(0,113,227,.12)",borderRadius:12,marginBottom:16,display:"flex",alignItems:"center",gap:12}}><div style={{width:32,height:32,borderRadius:8,background:stageOf(active.stage).color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{color:"#fff",fontSize:14,fontWeight:700}}>{stageOf(active.stage).icon}</span></div><div style={{minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:"var(--g900)"}}>Status: {stageOf(active.stage).label}</div><div style={{fontSize:12,color:"var(--g600)",marginTop:1}}>{stageOf(active.stage).guidance}</div></div></div>
...
<button onClick={()=>fRef.current?.click()} style={{padding:"10px 20px",borderRadius:100,fontSize:13,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer",marginBottom:12}}>+ Upload Documents</button>
<div>
{(active.documents||[]).map((d,i)=>{const dt=docTypeOf(d.tag);return(
<div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",marginBottom:4}}>
<div style={{width:32,height:32,borderRadius:8,background:`${dt.color}10`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
<span style={{fontSize:11,fontWeight:700,color:dt.color}}>{dt.id.slice(0,2).toUpperCase()}</span>
</div>
...
<button onClick={()=>{const newName=prompt("Rename document:",d.name);if(newName&&newName.trim()){const cl={...active};cl.documents=cl.documents.map((doc,j)=>j===i?{...doc,name:newName.trim(),tag:guessDocType(newName.trim())}:doc);saveClaim(cl)}}} style={{padding:"4px 10px",borderRadius:8,fontSize:11,fontWeight:500,border:"1px solid var(--card-border)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Rename</button>
{fc[d.name]&&<button onClick={()=>{const blob=new Blob([fc[d.name]],{type:"text/plain"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=d.name;a.click();URL.revokeObjectURL(url)}} style={{padding:"4px 10px",borderRadius:8,fontSize:11,fontWeight:500,border:"1px solid var(--card-border)",background:"#fff",color:"var(--blue)",cursor:"pointer"}}>Download</button>}
</div>)})}
</div>
...
<button onClick={()=>setEmailModal({to:"",subject:active.claimNumber+" - ",body:""})} style={{padding:"9px 18px",borderRadius:100,fontSize:13,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",gap:6}}><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75"/></svg>Send Email</button>
<button onClick={()=>setEmailModal({to:"",subject:active.claimNumber+" - Records Request",body:"Dear Dr. [Name],\n\nI am writing to request the medical records for "+active.worker+" (DOI: "+fmt(active.injuryDate)+") in connection with WSIB claim "+active.claimNumber+".\n\nPlease provide:\n- All clinical notes from date of injury to present\n- Diagnostic imaging reports\n- Treatment plans and referrals\n- Functional Abilities Form (FAF)\n\nThank you for your prompt attention to this matter.\n\nRegards"})} style={{padding:"9px 14px",borderRadius:100,fontSize:12,fontWeight:500,border:"1px solid var(--card-border)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Records Request</button>
<button onClick={()=>setEmailModal({to:"",subject:active.claimNumber+" - Status Update",body:"Hello,\n\nThis email is regarding WSIB claim "+active.claimNumber+" for "+active.worker+" ("+active.employer+").\n\nCurrent Status: "+stageOf(active.stage).label+"\n\n[Add your update here]\n\nPlease do not hesitate to reach out if you have any questions.\n\nRegards"})} style={{padding:"9px 14px",borderRadius:100,fontSize:12,fontWeight:500,border:"1px solid var(--card-border)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Status Update</button>
</div>
{/* EMAIL THREAD */}
{(active.emails||[]).length>0&&<div style={{marginBottom:14}}>
<div style={{fontSize:11,fontWeight:700,color:"var(--g500)",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Email Thread</div>
{(active.emails||[]).slice().reverse().map(em=>(
<div key={em.id} style={{padding:"14px 16px",background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",marginBottom:6}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
<div>
<div style={{display:"flex",alignItems:"center",gap:6}}>
<span style={{width:6,height:6,borderRadius:"50%",background:em.direction==="outbound"?"var(--blue)":"var(--green)"}}></span>
<span style={{fontSize:13,fontWeight:600,color:"var(--g800)"}}>{em.subject}</span>
</div>
<div style={{fontSize:11,color:"var(--g500)",marginTop:2}}>
{em.direction==="outbound"?"To":"From"}: {em.to}{em.cc?" | CC: "+em.cc:""}
</div>
</div>
<span style={{fontSize:10,color:"var(--g400)",whiteSpace:"nowrap"}}>{new Date(em.date).toLocaleDateString()+" "+new Date(em.date).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</span>
</div>
<div style={{fontSize:12,color:"var(--g600)",lineHeight:1.6,whiteSpace:"pre-wrap",padding:"10px 12px",background:"var(--g50)",borderRadius:8,maxHeight:120,overflowY:"auto"}}>{em.body}</div>
<div style={{display:"flex",gap:6,marginTop:8}}>
<button onClick={()=>setEmailModal({to:em.direction==="outbound"?em.to:em.from,subject:"Re: "+em.subject.replace(/^Re: /,""),body:"\n\n---\nOn "+new Date(em.date).toLocaleDateString()+", "+(em.direction==="outbound"?"you":"they")+" wrote:\n"+em.body})} style={{padding:"4px 12px",borderRadius:100,fontSize:11,fontWeight:500,border:"1px solid var(--card-border)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Reply</button>
<button onClick={()=>setEmailModal({to:"",subject:"Fwd: "+em.subject,body:"\n\n--- Forwarded ---\nFrom: "+em.from+"\nTo: "+em.to+"\nDate: "+new Date(em.date).toLocaleDateString()+"\nSubject: "+em.subject+"\n\n"+em.body})} style={{padding:"4px 12px",borderRadius:100,fontSize:11,fontWeight:500,border:"1px solid var(--card-border)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Forward</button>
</div>
</div>))}
</div>}
<div style={{fontSize:11,fontWeight:700,color:"var(--g500)",textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Communication Log</div>
...
<div><label style={{fontSize:11,fontWeight:600,color:"var(--g400)",display:"block",marginBottom:4}}>To/From</label><input value={commLog.to} onChange={e=>setCommLog(p=>({...p,to:e.target.value}))} placeholder="WSIB, Dr. Smith, Employer..." style={{width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid var(--card-border)",fontSize:13,outline:"none"}}/></div>
<div><label style={{fontSize:11,fontWeight:600,color:"var(--g400)",display:"block",marginBottom:4}}>Method</label><select value={commLog.method} onChange={e=>setCommLog(p=>({...p,method:e.target.value}))} style={{width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid var(--card-border)",fontSize:13,outline:"none",background:"#fff"}}><option value="email">Email</option><option value="phone">Phone</option><option value="mail">Mail</option><option value="portal">WSIB Portal</option><option value="meeting">Meeting</option></select></div>
</div>
<div style={{marginBottom:8}}><label style={{fontSize:11,fontWeight:600,color:"var(--g400)",display:"block",marginBottom:4}}>Date</label><input type="date" value={commLog.date} onChange={e=>setCommLog(p=>({...p,date:e.target.value}))} style={{width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid var(--card-border)",fontSize:13,outline:"none"}}/></div>
<div style={{marginBottom:8}}><label style={{fontSize:11,fontWeight:600,color:"var(--g400)",display:"block",marginBottom:4}}>Notes</label><input value={commLog.note} onChange={e=>setCommLog(p=>({...p,note:e.target.value}))} placeholder="Summary of communication..." style={{width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid var(--card-border)",fontSize:13,outline:"none"}}/></div>
<button onClick={()=>{if(commLog.to&&commLog.note){const cl={...active};cl.comms=[...(cl.comms||[]),{...commLog,id:Date.now()}];cl.timeline=[...(cl.timeline||[]),{date:new Date().toISOString(),type:"note",note:"Comm: "+commLog.method+" with "+commLog.to}];saveClaim(cl);setCommLog({to:"",method:"email",date:"",note:""})}}} style={{padding:"8px 16px",borderRadius:100,fontSize:13,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer"}}>Add Entry</button>
</div>
{(active.comms||[]).slice().reverse().map((entry,i)=><div key={i} style={{padding:"12px 16px",background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",marginBottom:6,display:"flex",alignItems:"flex-start",gap:12}}><div style={{width:8,height:8,borderRadius:"50%",background:entry.method==="email"?"var(--blue)":entry.method==="phone"?"var(--green)":"var(--orange)",marginTop:5,flexShrink:0}}/><div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:13,fontWeight:600,color:"var(--g800)"}}>{entry.to}</span><span style={{fontSize:11,color:"var(--g400)"}}>{entry.date}</span></div><div style={{fontSize:12,color:"var(--g500)",marginTop:2}}>{entry.note}</div><span style={{fontSize:10,fontWeight:600,color:"var(--g400)",textTransform:"uppercase"}}>{entry.method}</span></div></div>)}
...
<p style={{fontSize:14,color:"var(--g500)",maxWidth:460,margin:"0 auto",lineHeight:1.6}}>Ask about WSIB policy, get ruling predictions, analyze claim scenarios, or get guidance on the adjudication process.</p>
</div>}
