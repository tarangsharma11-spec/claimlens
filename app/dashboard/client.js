...
{id:"modified_duties_ai",category:"RTW",title:"Modified Duties Assessor",desc:"AI-generated return-to-work duties plan.",icon:"MD",color:"#0D7C5F",prompt:c=>{const docs=(c.documents||[]).map(d=>d.tag).join(", ")||"none";const existingDuties=(c.modifiedDuties||[]).map(d=>d.task||d.description).join("; ")||"none currently documented";const providers=(c.providers||[]).map(p=>p.name+" ("+p.type+")").join(", ")||"none listed";return`You are a Return-to-Work specialist analyzing claim ${c.claimNumber}.

WORKER: ${c.worker}
EMPLOYER: ${c.employer}
INJURY TYPE: ${c.injuryType}
DATE OF INJURY: ${c.injuryDate}
DAYS SINCE INJURY: ${c.injuryDate&&c.injuryDate!=="—"?Math.floor((Date.now()-new Date(c.injuryDate))/864e5):"unknown"}
DOCUMENTS ON FILE: ${docs}
TREATING PROVIDERS: ${providers}
CURRENT MODIFIED DUTIES: ${existingDuties}
DESCRIPTION: ${c.description||"N/A"}

Based on the injury type, duration, and available medical evidence, generate a comprehensive MODIFIED DUTIES ASSESSMENT following WSIB OPM 19-02-02 (Early and Safe Return to Work) guidelines.

Include the following sections:

## 1. FUNCTIONAL ABILITIES SUMMARY
Based on the injury type and duration, estimate the worker's likely functional abilities and restrictions:
- Physical: lifting capacity (lbs), standing/sitting duration, mobility restrictions
- Cognitive: concentration, memory, decision-making capacity
- Environmental: noise tolerance, temperature sensitivity, chemical exposure limits
- Hours: recommended work hours per day, gradual return schedule

## 2. RECOMMENDED MODIFIED DUTIES PLAN
Provide 4-6 specific modified duty tasks that:
- Match the worker's estimated functional abilities
- Are productive and meaningful (not "make-work")
- Progressively increase in demand over a 4-8 week timeline
- Are appropriate for the employer's industry (${c.employer})

For each duty, specify:
- Task description
- Physical demands (sedentary/light/medium)
- Duration per day
- Week to introduce (Week 1, Week 2, etc.)
- Accommodation needed

## 3. GRADUATED RETURN-TO-WORK SCHEDULE
Create a week-by-week schedule showing:
- Hours per day (starting at 4h, building to 8h)
- Duties for each phase
- Milestones for progression to next phase
- Expected full duties date

## 4. EMPLOYER OBLIGATIONS
List the employer's specific obligations under WSIB policy:
- Workplace accommodation requirements
- Communication requirements with WSIB and worker
- Documentation requirements
- Consequences of non-compliance (premium impacts)

## 5. RED FLAGS & RISKS
Identify potential RTW barriers:
- Re-injury risks for this injury type
- Accommodation gaps
- Timeline concerns
- When to escalate to a Work Transition Plan (WTP)

## 6. OPM POLICY REFERENCES
Cite specific OPM sections that apply:
- OPM 19-02-02 (ESRTW)
- OPM 19-02-04 (Work Transition Plans) if applicable
- Any injury-specific RTW policies

Format the duties plan so it can be directly shared with the employer as a formal RTW proposal.`}},
];
...
useEffect(()=>{async function fetchClaims(){try{const res=await fetch("/api/org/claims");if(res.ok){const data=await res.json();const cl=data.claims||[];setClaims(cl);return cl}else{const cl=loadClaims(user.email);setClaims(cl);return cl}}catch{const cl=loadClaims(user.email);setClaims(cl);return cl}}async function fetchMembers(){try{const res=await fetch("/api/org/members");if(res.ok){const data=await res.json();setOrgMembers(data.members||[])}}catch{}}fetchClaims().then(cl=>{
...
}}})});if((cl||[]).length===0&&!localStorage.getItem("ca_onboarded_"+user.email))setShowOnboarding(true)});fetchMembers()},[user.email]);
...
const WORKFLOW_AI_PROMPTS={
report:(c)=>({title:"Incident Report Analysis",prompt:`Analyze the injury report for claim ${c.claimNumber}. Worker: ${c.worker}, Employer: ${c.employer}, Type: ${c.injuryType}, DOI: ${c.injuryDate}. Description: ${c.description||"N/A"}. Documents: ${(c.documents||[]).map(d=>d.tag).join(", ")||"none"}.\n\nGenerate:\n1. A clear incident summary suitable for WSIB Form 6/7 submission\n2. Key facts extracted (who, what, when, where, how)\n3. Missing information that should be gathered\n4. Filing compliance check (was Form 6 filed within worker deadline? Form 7 within 3 business days?)\n5. Initial risk assessment`}),
contact:(c)=>({title:"Three-Point Contact Plan",prompt:`Generate a Three-Point Contact communication plan for claim ${c.claimNumber}. Worker: ${c.worker}, Employer: ${c.employer}, Type: ${c.injuryType}, DOI: ${c.injuryDate}.\n\nFor each of the three contacts (Worker, Employer, Healthcare Provider), provide:\n1. Recommended contact method (phone/email/in-person)\n2. Key questions to ask\n3. Information to communicate\n4. Documentation requirements\n5. Follow-up timeline\n\nReference OPM co-operation obligation requirements. Include template scripts for each call.`}),
medical:(c)=>({title:"Medical Evidence Review",prompt:`Review the medical evidence for claim ${c.claimNumber}. Type: ${c.injuryType}. Documents: ${(c.documents||[]).map(d=>d.name+" ["+d.tag+"]").join("; ")||"none"}. Providers: ${(c.providers||[]).map(p=>p.name+" ("+p.type+")").join(", ")||"none"}.\n\nGenerate:\n1. Summary of medical evidence on file\n2. Evidence gaps — what's missing?\n3. Recommended additional evidence to obtain\n4. Which providers to request reports from\n5. Consistency check — do the medical reports align?\n6. Form 8 (Functional Abilities) analysis if available`}),
analyze:(c)=>({title:"Five Point Check Analysis",prompt:`Run a full Five Point Check adjudication analysis on claim ${c.claimNumber}. Worker: ${c.worker}, Employer: ${c.employer}, Type: ${c.injuryType}, DOI: ${c.injuryDate}. Description: ${c.description||"N/A"}. Documents: ${(c.documents||[]).map(d=>d.tag).join(", ")||"none"}.\n\nApply the Five Point Check:\n1. Was there a personal injury?\n2. Did an accident arise out of employment?\n3. Did the accident arise in the course of employment?\n4. Was the disability caused by the accident?\n5. Is there a pre-existing condition (thin skull/crumbling skull)?\n\nProvide a RULING PREDICTION with confidence level and cite relevant OPM policies.`}),
decision:(c)=>({title:"Decision Review",prompt:`Review the decision for claim ${c.claimNumber}. Type: ${c.injuryType}. Latest ruling: ${c.analyses?.[c.analyses.length-1]?.ruling||"none"}. Stage: ${c.stage}.\n\nGenerate:\n1. Decision summary and rationale\n2. Strengths of the current ruling\n3. Potential grounds for appeal if denied\n4. Next steps based on the decision\n5. Timeline for appeal (6 months from decision date)\n6. Relevant WSIAT precedents for this type of claim`}),
rtw:(c)=>({title:"Return-to-Work Plan",prompt:AI_TOOLS.find(t=>t.id==="modified_duties_ai")?.prompt(c)||"Generate a return-to-work plan for this claim."}),
resolve:(c)=>({title:"Close or Appeal Assessment",prompt:`Assess whether claim ${c.claimNumber} should be closed or appealed. Type: ${c.injuryType}. Stage: ${c.stage}. Ruling: ${c.analyses?.[c.analyses.length-1]?.ruling||"none"}. Modified duties: ${(c.modifiedDuties||[]).length} documented.\n\nGenerate:\n1. Claim resolution readiness assessment\n2. If denied: appeal viability score (0-100) with reasoning\n3. If allowed: closure checklist\n4. Outstanding items that must be resolved\n5. Financial summary (estimated benefits, costs)\n6. Recommended next action with timeline`}),
};
const runWorkflowAI=async(stepId)=>{if(!active)return;const config=WORKFLOW_AI_PROMPTS[stepId];if(!config)return;const{title,prompt}=config(active);setWfAi({open:true,stepId,loading:true,result:null,title});try{const hist=[{role:"user",content:prompt}];const res=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:hist.map(m=>({...m,content:redactPII(m.content,privacyLevel)}))})});const data=await res.json();if(!res.ok)throw new Error(data.error||"AI request failed");const reply=data.reply;setWfAi(p=>({...p,loading:false,result:reply}));/* Save to case */const c={...active};c.analyses=[...(c.analyses||[]),{date:new Date().toISOString(),tool:"workflow_"+stepId,title,snippet:reply.slice(0,300),fullText:reply}];c.timeline=[...(c.timeline||[]),{date:new Date().toISOString(),type:"ai_tool",note:title+" completed",tool:"workflow_"+stepId}];c.messages=[...(c.messages||[]),{role:"user",content:prompt,ts:new Date().toISOString()},{role:"assistant",content:reply,ts:new Date().toISOString()}];saveClaim(c)}catch(err){setWfAi(p=>({...p,loading:false,result:"**Error:** "+err.message}));showToast(err.message)}};
const saveClaim=(c)=>{c.updatedAt=new Date().toISOString();c.ownerEmail=user.email.toLowerCase();setClaims(p=>{const idx=p.findIndex(x=>x.id===c.id);const n=[...p];if(idx>=0)n[idx]=c;else n.unshift(c);return n});setActive(c);/* Persist to DB */fetch("/api/org/claims",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)}).catch(err=>{console.log("Save failed:",err.message);showToast("Failed to save — changes may be lost. Check your connection.")})};
...
const[mentionQ,setMentionQ]=useState("");const[showMentions,setShowMentions]=useState(false);const[mentionPos,setMentionPos]=useState(null);
const addNote=()=>{if(!noteIn.trim())return;const text=noteIn.trim();const c={...active,notes:[...(active.notes||[]),{date:new Date().toISOString(),text,author:user?.name||user?.email}],timeline:[...(active.timeline||[]),{date:new Date().toISOString(),type:"note",note:text,author:user?.name||user?.email}]};saveClaim(c);/* Send @mention notifications */const mentions=text.match(/@(\S+)/g)||[];mentions.forEach(m=>{const name=m.slice(1).toLowerCase();const member=orgMembers.find(om=>(om.user_name||"").toLowerCase().includes(name)||(om.user_email||"").toLowerCase().includes(name));if(member&&member.user_email!==user?.email){fetch("/api/email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:member.user_email,subject:`${user?.name||user?.email} mentioned you on ${active.claimNumber}`,body:`Hi ${member.user_name||member.user_email},\n\n${user?.name||user?.email} mentioned you in a note on case ${active.claimNumber}:\n\n"${text}"\n\nView the case at https://www.caseassist.ca/dashboard\n\n— CaseAssist`,senderName:user?.name||"CaseAssist",senderEmail:user?.email})}).catch(()=>{})}});setNoteIn("")};
const handleNoteInput=(val)=>{setNoteIn(val);const lastAt=val.lastIndexOf("@");if(lastAt>=0&&(lastAt===0||val[lastAt-1]===" ")){const q=val.slice(lastAt+1).toLowerCase();if(q.length>0&&!q.includes(" ")){setMentionQ(q);setShowMentions(true);return}}setShowMentions(false)};
const insertMention=(member)=>{const lastAt=noteIn.lastIndexOf("@");const before=noteIn.slice(0,lastAt);const name=(member.user_name||member.user_email).split(" ")[0];setNoteIn(before+"@"+name+" ");setShowMentions(false)};
...
/* Auto-extract modified duties from AI Modified Duties Assessor */
if(reply.toLowerCase().includes("modified dut")&&reply.toLowerCase().includes("week")){const dutyMatches=reply.match(/[-•]\s*(?:Task|Duty)?:?\s*([^\n]{10,80})/gi)||[];if(dutyMatches.length>0&&(!c.modifiedDuties||c.modifiedDuties.length===0)){const duties=dutyMatches.slice(0,8).map((d,idx)=>({id:Date.now()+idx,task:d.replace(/^[-•]\s*(?:Task|Duty)?:?\s*/i,"").trim(),demand:"light",hours:"4",week:Math.ceil((idx+1)/2),status:"proposed",addedAt:new Date().toISOString(),source:"ai"}));c.modifiedDuties=[...(c.modifiedDuties||[]),...duties];c.timeline=[...(c.timeline||[]),{date:new Date().toISOString(),type:"ai_tool",note:`${duties.length} modified duties auto-generated from AI assessment`,tool:"modified_duties_ai"}]}}
...
{/* === GROUPED NAV === */}
{(()=>{
const navHandler=(id)=>{if(id==="advisor"){setActive(null);setMsgs([]);nav("chat")}else if(id==="team")setShowAdminPanel(true);else if(id==="templates")setShowTemplates(true);else if(id==="analytics")setShowAnalytics(true);else if(id==="glossary")setShowGlossary(true);else if(id==="intake")setShowIntake(true);else if(id==="employer")setShowEmployerDash(true);else if(id==="triage")setShowTriageQueue(true);else if(id==="worker")setShowWorkerPortal(true);else if(id==="settlement")setShowSettlement(true);else if(id==="limitations")setShowLimitations(true);else if(id==="emptools")setShowEmployerTools(true);else if(id==="adjintel")setShowAdjudicatorIntel(true);else nav(id)};
...
<button onClick={()=>setShowAdminPanel(true)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px",margin:"1px 8px",borderRadius:10,border:"none",background:"transparent",color:"rgba(255,255,255,.5)",cursor:"pointer",overflow:"hidden",whiteSpace:"nowrap"}}><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8" style={{flexShrink:0}}><path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z"/></svg>{sideHover&&<span style={{fontSize:12,fontWeight:500,color:"rgba(255,255,255,.6)"}}>Team</span>}</button>
...
<button onClick={()=>setModal("new")} style={{padding:"7px 16px",borderRadius:8,fontSize:12,fontWeight:600,border:"none",background:"linear-gradient(135deg, #1A1040, #3B5EC0)",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",gap:5}}><svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15"/></svg>New Case</button>
<div style={{position:"relative"}}><button onClick={()=>setModal(modal==="avatar"?null:"avatar")} style={{width:30,height:30,borderRadius:"50%",background:"linear-gradient(135deg, #1A1040, #3B5EC0)",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,cursor:"pointer",border:"none"}}>{(user.name||user.email||"U").charAt(0).toUpperCase()}</button>{modal==="avatar"&&<div style={{position:"absolute",top:36,right:0,width:200,background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"0 8px 24px rgba(0,0,0,.1)",zIndex:50,padding:8,animation:"scaleIn .15s ease"}}><div style={{padding:"8px 10px",fontSize:12,fontWeight:600,color:"var(--g700)",borderBottom:"1px solid var(--g100)",marginBottom:4}}>{user.name||user.email}</div><button onClick={()=>{setShowAdminPanel(true);setModal(null)}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",borderRadius:8,border:"none",background:"transparent",cursor:"pointer",fontSize:13,color:"var(--g700)",fontFamily:"inherit"}}>Team</button><button onClick={()=>{setShowBilling(true);setModal(null)}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",borderRadius:8,border:"none",background:"transparent",cursor:"pointer",fontSize:13,color:"var(--g700)",fontFamily:"inherit"}}>Billing</button><button onClick={()=>signOut({callbackUrl:"/login"})} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",borderRadius:8,border:"none",background:"transparent",cursor:"pointer",fontSize:13,color:"var(--red)",fontFamily:"inherit",marginTop:4,borderTop:"1px solid var(--g100)",paddingTop:12}}>Sign Out</button></div>}</div>
</div>
...
{/* Gong-style Case Details Card */}
<div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:4}}>
<button onClick={()=>nav("claims")} style={{fontSize:11,fontWeight:500,color:"var(--blue)",background:"none",border:"none",cursor:"pointer",padding:0,marginBottom:6,display:"block"}}>{"<-"} My Cases</button>
...
<div style={{position:"relative"}}><button onClick={()=>setModal(modal==="assign"?null:"assign")} title="Assign" style={{width:32,height:32,borderRadius:8,border:"1px solid var(--card-border)",background:active.assignedTo?"var(--blue-light)":"#fff",color:active.assignedTo?"var(--blue)":"var(--g500)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/></svg></button>{modal==="assign"&&<div style={{position:"absolute",top:36,right:0,width:220,background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"0 8px 24px rgba(0,0,0,.1)",zIndex:50,padding:4,animation:"scaleIn .15s ease"}}><div style={{padding:"8px 10px",fontSize:11,fontWeight:600,color:"var(--g400)",textTransform:"uppercase",letterSpacing:.5}}>Assign to</div>{orgMembers.filter(m=>m.status!=="disabled").map(m=><button key={m.user_email} onClick={()=>{const cl={...active,assignedTo:m.user_email,assignee:m.user_name||m.user_email,timeline:[...(active.timeline||[]),{date:new Date().toISOString(),type:"note",note:"Assigned to "+(m.user_name||m.user_email)}]};saveClaim(cl);setModal(null)}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",borderRadius:8,border:"none",background:active.assignedTo===m.user_email?"var(--blue-light)":"transparent",cursor:"pointer",fontSize:13,color:"var(--g700)",fontFamily:"inherit",textAlign:"left"}}><div style={{width:24,height:24,borderRadius:"50%",background:active.assignedTo===m.user_email?"var(--blue)":"var(--g200)",color:active.assignedTo===m.user_email?"#fff":"var(--g600)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,flexShrink:0}}>{(m.user_name||m.user_email)[0]?.toUpperCase()}</div><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.user_name||m.user_email}</span></button>)}<button onClick={()=>{const cl={...active,assignedTo:null,assignee:null,timeline:[...(active.timeline||[]),{date:new Date().toISOString(),type:"note",note:"Unassigned"}]};saveClaim(cl);setModal(null)}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",borderRadius:8,border:"none",background:"transparent",cursor:"pointer",fontSize:13,color:"var(--g400)",fontFamily:"inherit",marginTop:2,borderTop:"1px solid var(--g100)"}}>Unassign</button></div>}</div><div style={{position:"relative"}}><button onClick={()=>setModal(modal==="share"?null:"share")} title="Share" style={{width:32,height:32,borderRadius:8,border:"1px solid var(--card-border)",background:(active.sharedWith||[]).length>0?"#E8F5E9":"#fff",color:(active.sharedWith||[]).length>0?"#28A745":"var(--g500)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 103.935 2.186 2.25 2.25 0 00-3.935-2.186zm0-12.814a2.25 2.25 0 103.933-2.185 2.25 2.25 0 00-3.933 2.185z"/></svg></button>{modal==="share"&&<div style={{position:"absolute",top:36,right:0,width:240,background:"#fff",borderRadius:12,border:"1px solid var(--card-border)",boxShadow:"0 8px 24px rgba(0,0,0,.1)",zIndex:50,padding:4,animation:"scaleIn .15s ease"}}><div style={{padding:"8px 10px",fontSize:11,fontWeight:600,color:"var(--g400)",textTransform:"uppercase",letterSpacing:.5}}>Share with</div>{orgMembers.filter(m=>m.status!=="disabled"&&m.user_email!==user?.email).map(m=>{const isShared=(active.sharedWith||[]).includes(m.user_email);return<button key={m.user_email} onClick={()=>{fetch("/api/org/claims",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:active.id,action:isShared?"unshare":"share",shareWith:m.user_email})}).then(()=>{const newShared=isShared?(active.sharedWith||[]).filter(e=>e!==m.user_email):[...(active.sharedWith||[]),m.user_email];const cl={...active,sharedWith:newShared};setActive(cl);setClaims(p=>p.map(c=>c.id===cl.id?cl:c))}).catch(()=>{})}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",borderRadius:8,border:"none",background:isShared?"#E8F5E9":"transparent",cursor:"pointer",fontSize:13,color:isShared?"#28A745":"var(--g700)",fontFamily:"inherit",textAlign:"left"}}><div style={{width:24,height:24,borderRadius:"50%",background:isShared?"#28A745":"var(--g200)",color:isShared?"#fff":"var(--g600)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,flexShrink:0}}>{isShared?"✓":(m.user_name||m.user_email)[0]?.toUpperCase()}</div><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.user_name||m.user_email}</span></button>})}</div>}</div><button title={timer.running&&timer.caseId===active.id?"Stop Timer":"Start Timer"} onClick={()=>{if(timer.running&&timer.caseId===active.id){const elapsed=Math.floor((Date.now()-timer.start)/60000);setClaims(prev=>{const next=prev.map(cc=>{if(cc.id===active.id){return{...cc,timeEntries:[...(cc.timeEntries||[]),{date:new Date().toISOString(),minutes:elapsed,desc:"Timed session"}],timeline:[...(cc.timeline||[]),{date:new Date().toISOString(),type:"note",note:"Time logged: "+elapsed+" minutes"}]}}return cc});if(active)setActive(next.find(x=>x.id===active.id));return next});setTimer({running:false,caseId:null,start:null,entries:[]})}else{setTimer({running:true,caseId:active.id,start:Date.now(),entries:[]})}}} style={{width:32,height:32,borderRadius:8,border:"1px solid var(--card-border)",background:timer.running&&timer.caseId===active.id?"var(--red-light)":"#fff",color:timer.running&&timer.caseId===active.id?"var(--red)":"var(--g500)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></button>
</div>
</div>

{/* Gong-style Case Details Card */}
<div style={{padding:"16px 20px",background:"#fff",borderRadius:16,border:"1px solid var(--card-border)",boxShadow:"var(--card-shadow)",marginBottom:14}}>
<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
<div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg, #1A1040, #3B5EC0)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:15,fontWeight:800,flexShrink:0}}>{(active.worker||"?")[0]?.toUpperCase()}</div>
<div style={{flex:1,minWidth:0}}>
<div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
<span style={{fontSize:20,fontWeight:800,letterSpacing:-.5}}>{active.claimNumber}</span>
...
{active.analyses?.length>0&&(()=>{const lr=active.analyses[active.analyses.length-1].ruling;return<span style={{padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:600,color:lr==="Allow"?"#1B7D36":lr==="Deny"?"#C62828":"#E65100",background:lr==="Allow"?"rgba(40,167,69,.06)":lr==="Deny"?"rgba(229,57,53,.06)":"rgba(245,124,0,.06)"}}>AI: {lr}</span>})()}
</div>
</div>
</div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px 16px"}}>
{[
{label:"Worker",value:active.worker,field:"worker"},
{label:"Employer",value:active.employer,field:"employer"},
{label:"Injury Type",value:active.injuryType,field:"injuryType"},
{label:"Date of Injury",value:active.injuryDate&&active.injuryDate!=="—"?fmt(active.injuryDate):"—"},
{label:"Days Open",value:active.injuryDate&&active.injuryDate!=="—"?"Day "+daysAgo(active.injuryDate):"—"},
{label:"Assigned To",value:active.assignee||"Unassigned"},
{label:"Documents",value:(active.documents?.length||0)+" files"},
{label:"Analyses",value:(active.analyses?.length||0)+" runs"},
{label:"Created",value:active.createdAt?new Date(active.createdAt).toLocaleDateString():"—"},
].map((item,i)=><div key={i} style={{padding:"6px 0"}}>
<div style={{fontSize:10,fontWeight:600,color:"var(--g400)",textTransform:"uppercase",letterSpacing:.5,marginBottom:2}}>{item.label}</div>
{item.field&&inlineEdit===item.field?<input autoFocus value={inlineVal} onChange={e=>setInlineVal(e.target.value)} onBlur={()=>{if(inlineVal.trim()){const cl={...active};cl[item.field]=inlineVal.trim();saveClaim(cl)}setInlineEdit(null)}} onKeyDown={e=>{if(e.key==="Enter"){if(inlineVal.trim()){const cl={...active};cl[item.field]=inlineVal.trim();saveClaim(cl)}setInlineEdit(null)}if(e.key==="Escape")setInlineEdit(null)}} style={{fontSize:13,fontWeight:600,border:"none",borderBottom:"2px solid var(--blue)",outline:"none",background:"transparent",color:"var(--g800)",width:"100%",padding:0,fontFamily:"inherit"}}/>:
<div onClick={()=>{if(item.field){setInlineEdit(item.field);setInlineVal(item.value||"")}}} style={{fontSize:13,fontWeight:600,color:"var(--g800)",cursor:item.field?"pointer":"default"}} title={item.field?"Click to edit":""}>{item.value||"—"}</div>}
</div>)}
</div>
</div>
...
<button onClick={()=>{if(current.actionNav==="chat"){setMsgs(active.messages||[]);nav("chat")}else setDetailTab(current.actionNav||"overview")}} style={{padding:"8px 16px",borderRadius:100,fontSize:12,fontWeight:600,border:"1px solid var(--g300)",background:"#fff",color:"var(--g600)",cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
...
Go to step</button>
{WORKFLOW_AI_PROMPTS[current.id]&&<button onClick={()=>runWorkflowAI(current.id)} style={{padding:"8px 16px",borderRadius:100,fontSize:12,fontWeight:600,border:"none",background:"linear-gradient(135deg, #1A1040, #3B5EC0)",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
...
const color=ev.type==="document"?"var(--green)":ev.type==="analysis"?"#F57C00":ev.type==="ai_tool"?"#6C5CE7":ev.type==="stage"?"#6C5CE7":ev.type==="email"?"var(--blue)":"var(--g400)";
...
{detailTab==="timeline"&&<>
<div style={{marginBottom:16}}>{(active.timeline||[]).slice().reverse().map((ev,i,arr)=><div key={i} style={{display:"flex",gap:10,position:"relative"}}>{i<arr.length-1&&<div style={{position:"absolute",left:10,top:22,bottom:0,width:1,background:"var(--g200)"}}/>}<div style={{width:22,height:22,borderRadius:"50%",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,zIndex:1,background:ev.type==="stage"?"var(--blue-light)":ev.type==="document"?"var(--green-light)":ev.type==="analysis"?"rgba(255,149,0,.06)":ev.type==="ai_tool"?"rgba(108,92,231,.06)":ev.type==="note"?"rgba(175,82,222,.06)":"var(--g200)",color:ev.type==="stage"?"var(--blue)":ev.type==="document"?"var(--green)":ev.type==="analysis"?"var(--orange)":ev.type==="ai_tool"?"#6C5CE7":ev.type==="note"?"#AF52DE":"var(--g600)"}}>{ev.type==="created"?"●":ev.type==="stage"?"→":ev.type==="document"?"+":ev.type==="analysis"?"✷":ev.type==="ai_tool"?"⚡":"•"}</div><div style={{flex:1,paddingBottom:14,minWidth:0}}><div style={{fontSize:13,color:"var(--g700)",lineHeight:1.4}}>{ev.note}{ev.author&&<span style={{fontSize:11,color:"var(--g400)"}}> — {ev.author}</span>}</div>{ev.type==="ai_tool"&&ev.tool&&<button onClick={()=>{const analysis=(active.analyses||[]).find(a=>a.tool===ev.tool&&a.date===ev.date);if(analysis?.fullText){setExpandedActivity(expandedActivity===ev.date?null:ev.date)}}} style={{fontSize:11,fontWeight:600,color:"#6C5CE7",background:"none",border:"none",cursor:"pointer",padding:"4px 0",marginTop:2}}>
{expandedActivity===ev.date?"Hide output ▲":"View AI output ▼"}</button>}
{expandedActivity===ev.date&&(()=>{const analysis=(active.analyses||[]).find(a=>a.tool===ev.tool&&a.date===ev.date);return analysis?.fullText?<div style={{marginTop:6,padding:"10px 12px",background:"var(--g50)",borderRadius:8,border:"1px solid var(--g100)",fontSize:12,color:"var(--g700)",lineHeight:1.5,maxHeight:300,overflowY:"auto",whiteSpace:"pre-wrap"}}>{analysis.fullText}</div>:null})()}
<div style={{fontSize:11,color:"var(--g400)",marginTop:2}}>{fmtTime(ev.date)}</div></div></div>)}</div>
<div style={{display:"flex",gap:6,position:"relative"}}><input value={noteIn} onChange={e=>handleNoteInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!showMentions)addNote();if(e.key==="Escape")setShowMentions(false)}} placeholder="Add a note… use @ to mention" style={{flex:1,padding:"8px 12px",borderRadius:12,border:"1px solid var(--card-border)",fontSize:13,outline:"none",background:"#fff"}}/><button onClick={addNote} disabled={!noteIn.trim()} style={{padding:"7px 14px",borderRadius:100,fontSize:13,fontWeight:600,border:"1px solid var(--g300)",background:"transparent",color:"var(--g600)",cursor:"pointer",whiteSpace:"nowrap"}}>Add</button>{showMentions&&<div style={{position:"absolute",bottom:"100%",left:0,width:220,background:"#fff",borderRadius:10,border:"1px solid var(--card-border)",boxShadow:"0 4px 16px rgba(0,0,0,.1)",zIndex:50,padding:4,marginBottom:4}}>{orgMembers.filter(m=>m.status!=="disabled"&&((m.user_name||"").toLowerCase().includes(mentionQ)||(m.user_email||"").toLowerCase().includes(mentionQ))).slice(0,5).map(m=><button key={m.user_email} onClick={()=>insertMention(m)} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"6px 8px",borderRadius:6,border:"none",background:"transparent",cursor:"pointer",fontSize:12,color:"var(--g700)",fontFamily:"inherit",textAlign:"left"}}><div style={{width:20,height:20,borderRadius:"50%",background:"var(--g200)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700}}>{(m.user_name||m.user_email)[0]?.toUpperCase()}</div>{m.user_name||m.user_email}</button>)}{orgMembers.filter(m=>m.status!=="disabled"&&((m.user_name||"").toLowerCase().includes(mentionQ)||(m.user_email||"").toLowerCase().includes(mentionQ))).length===0&&<div style={{padding:"6px 8px",fontSize:12,color:"var(--g400)"}}>No matches</div>}</div>}</div>
</>}
...
<div style={{flex:1}}/>
<input value={noteIn} onChange={e=>handleNoteInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&noteIn.trim()&&!showMentions)addNote()}} placeholder="Add a note... use @ to mention" style={{flex:1,maxWidth:240,padding:"8px 12px",borderRadius:10,border:"1px solid var(--card-border)",fontSize:12,outline:"none",background:"#fff"}}/>
...
{["Assessment","Legal","Adjudication","Medical","RTW","Reference"].map(cat=>{const tools=AI_TOOLS.filter(t=>t.category===cat);if(tools.length===0)return null;return(<div key={cat} style={{marginBottom:16}}>
...
{wfAi.open&&<div onClick={e=>{if(e.target===e.currentTarget&&!wfAi.loading)setWfAi({open:false,stepId:null,loading:false,result:null,title:""})}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",backdropFilter:"blur(8px)",zIndex:210,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
<div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:680,maxHeight:"85vh",display:"flex",flexDirection:"column",boxShadow:"0 20px 60px rgba(0,0,0,.2)",animation:"scaleIn .3s ease"}}>
<div style={{padding:"20px 24px",borderBottom:"1px solid var(--g100)",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
<div>
<div style={{fontSize:16,fontWeight:700,color:"var(--g900)"}}>{wfAi.title}</div>
<div style={{fontSize:12,color:"var(--g500)",marginTop:2}}>{active?.claimNumber} · {active?.worker}</div>
</div>
{!wfAi.loading&&<button onClick={()=>setWfAi({open:false,stepId:null,loading:false,result:null,title:""})} style={{background:"none",border:"none",cursor:"pointer",color:"var(--g400)",fontSize:20}}>×</button>}
</div>
<div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
{wfAi.loading?<div style={{textAlign:"center",padding:"48px 0"}}>
<div style={{width:48,height:48,borderRadius:"50%",border:"3px solid var(--g200)",borderTopColor:"var(--blue)",animation:"spin 1s linear infinite",margin:"0 auto 16px"}}/>
<div style={{fontSize:15,fontWeight:600,color:"var(--g700)",marginBottom:4}}>Analyzing...</div>
<div style={{fontSize:12,color:"var(--g500)"}}>Running {wfAi.title} on {active?.claimNumber}</div>
<style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
</div>:wfAi.result?<div style={{fontSize:13,color:"var(--g700)",lineHeight:1.7,whiteSpace:"pre-wrap"}}>{wfAi.result}</div>:null}
</div>
{wfAi.result&&!wfAi.loading&&<div style={{padding:"14px 24px",borderTop:"1px solid var(--g100)",display:"flex",gap:8,justifyContent:"flex-end",flexShrink:0}}>
<button onClick={()=>{const blob=new Blob([wfAi.result],{type:"text/plain"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=(wfAi.title||"AI-Analysis").replace(/\s+/g,"-")+"-"+(active?.claimNumber||"")+".txt";a.click();URL.revokeObjectURL(url)}} style={{padding:"8px 16px",borderRadius:10,fontSize:12,fontWeight:600,border:"1px solid var(--g300)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Download .txt</button>
<button onClick={()=>{navigator.clipboard?.writeText(wfAi.result);showToast("Copied to clipboard","success")}} style={{padding:"8px 16px",borderRadius:10,fontSize:12,fontWeight:600,border:"1px solid var(--g300)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Copy</button>
<button onClick={()=>setWfAi({open:false,stepId:null,loading:false,result:null,title:""})} style={{padding:"8px 16px",borderRadius:10,fontSize:12,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer"}}>Done</button>
</div>}
</div>
</div>}
{toast&&<div style={{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",padding:"12px 20px",borderRadius:12,background:toast.type==="error"?"#1D1D1F":"#28A745",color:"#fff",fontSize:13,fontWeight:600,boxShadow:"0 8px 32px rgba(0,0,0,.2)",zIndex:999,display:"flex",alignItems:"center",gap:8,animation:"scaleIn .2s ease",maxWidth:"90vw"}}><span>{toast.type==="error"?"⚠":"✓"}</span>{toast.msg}<button onClick={()=>setToast(null)} style={{background:"none",border:"none",color:"rgba(255,255,255,.6)",cursor:"pointer",fontSize:16,marginLeft:8}}>×</button></div>}
