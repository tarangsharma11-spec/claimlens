...
useEffect(()=>{const cl=loadClaims(user.email);setClaims(cl);
...
{getNavItems(userRole||"lawyer",userPlan).filter(t=>hasAccess(userRole,t.id)).map(t=>(
<button key={t.id} onClick={()=>{if(t.id==="advisor"){setActive(null);setMsgs([]);nav("chat")}else if(t.id==="templates")setShowTemplates(true);else if(t.id==="analytics")setShowAnalytics(true);else if(t.id==="glossary")setShowGlossary(true);else if(t.id==="intake")setShowIntake(true);else if(t.id==="employer")setShowEmployerDash(true);else if(t.id==="triage")setShowTriageQueue(true);else if(t.id==="worker")setShowWorkerPortal(true);else if(t.id==="settlement")setShowSettlement(true);else if(t.id==="limitations")setShowLimitations(true);else if(t.id==="emptools")setShowEmployerTools(true);else if(t.id==="adjintel")setShowAdjudicatorIntel(true);else nav(t.id)}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px",margin:"1px 8px",borderRadius:10,border:"none",background:(view===t.id||(t.id==="advisor"&&view==="chat"&&!active))?"rgba(255,255,255,.2)":"transparent",color:(view===t.id||(t.id==="advisor"&&view==="chat"&&!active))?"#fff":"rgba(255,255,255,.6)",cursor:"pointer",overflow:"hidden",whiteSpace:"nowrap"}}>
...
<button onClick={()=>{if(userRole==="admin")setShowAdminPanel(true)}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px",margin:"6px 8px 1px",borderRadius:10,border:"none",background:"rgba(255,255,255,.06)",color:"rgba(255,255,255,.45)",cursor:userRole==="admin"?"pointer":"default",overflow:"hidden",whiteSpace:"nowrap"}}><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8" style={{flexShrink:0}}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0"/></svg>{sideHover&&<span style={{fontSize:11,fontWeight:500}}>{ROLES[userRole]?.label||"User"}{userRole==="admin"?" — Manage":"" }</span>}</button>
<div style={{flex:1}}/>
...
{/* WORKFLOW MODALS */}
{showIntake&&<IntakeScreener user={user} onCreateCase={(claim)=>{setClaims(p=>{const next=[claim,...p];persistClaims(user.email,next);return next});setActive(claim);setView("detail");setShowIntake(false)}} onClose={()=>setShowIntake(false)}/>}
