...
{label:"Acute Back Injury",type:"Acute Injury",desc:"Worker sustained acute lumbar strain/sprain from lifting, bending, or twisting. Sudden onset of lower back pain.",icon:"🏗️"},
{label:"Slip & Fall",type:"Acute Injury",desc:"Worker slipped/tripped and fell at the workplace. May involve multiple body parts.",icon:"⚠️"},
{label:"Repetitive Strain",type:"Occupational Disease",desc:"Gradual onset of pain/dysfunction from repetitive workplace tasks. Common in upper extremity, shoulder, wrist.",icon:"🔁"},
{label:"Occupational Hearing Loss",type:"Occupational Disease",desc:"Progressive hearing loss from prolonged exposure to workplace noise. Audiometric evidence required.",icon:"👂"},
{label:"First Responder PTSD",type:"PTSD (First Responder)",desc:"PTSD claim from first responder (police, fire, paramedic) with presumptive coverage under OPM 15-03-13.",icon:"🚑"},
{label:"Workplace Mental Stress",type:"Traumatic Mental Stress",desc:"Acute or chronic mental stress arising from workplace events or conditions.",icon:"🧠"},
{label:"Pre-existing Aggravation",type:"Aggravation of Pre-existing",desc:"Workplace incident aggravated a pre-existing condition. Thin skull principle may apply.",icon:"⚕️"},
{label:"Recurrence",type:"Recurrence",desc:"Return of symptoms from a previously accepted WSIB claim. Must show causal connection to original injury.",icon:"🔄"},
];
...
<nav style={{height:64,padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(250,251,252,.85)",backdropFilter:"saturate(180%) blur(20px)",borderBottom:"1px solid rgba(0,0,0,.04)",flexShrink:0,zIndex:100}}>
  ...
  <div style={{display:"flex",gap:6,alignItems:"center"}}><button onClick={()=>setShowNotifs(!showNotifs)} style={{padding:"7px 10px",borderRadius:980,fontSize:13,border:".5px solid var(--g300)",background:showNotifs?"var(--g900)":"transparent",color:showNotifs?"#fff":"var(--g500)",cursor:"pointer",position:"relative"}}>🔔</button><button onClick={()=>setShowGlossary(!showGlossary)} style={{padding:"7px 10px",borderRadius:980,fontSize:13,border:".5px solid var(--g300)",background:showGlossary?"var(--g900)":"transparent",color:showGlossary?"#fff":"var(--g500)",cursor:"pointer"}}>📖</button><button onClick={()=>setShowTemplates(true)} style={{padding:"7px 10px",borderRadius:980,fontSize:11,fontWeight:500,border:".5px solid var(--g300)",background:"transparent",color:"var(--g500)",cursor:"pointer",whiteSpace:"nowrap"}} title="Templates">📋</button><button onClick={()=>setShowAnalytics(true)} style={{padding:"7px 10px",borderRadius:980,fontSize:11,fontWeight:500,border:".5px solid var(--g300)",background:"transparent",color:"var(--g500)",cursor:"pointer",whiteSpace:"nowrap"}} title="Analytics">📊</button><button onClick={()=>setModal("new")} style={{padding:"7px 14px",borderRadius:980,fontSize:12,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer",whiteSpace:"nowrap"}}>+ New Case</button><button onClick={()=>signOut({callbackUrl:"/login"})} style={{padding:"7px 10px",borderRadius:980,fontSize:11,fontWeight:500,border:".5px solid var(--g300)",background:"transparent",color:"var(--g500)",cursor:"pointer"}}>Sign Out</button></div>
</nav>

{/* MOBILE NAV */}
<div className="mobile-bnav" style={{display:"none",position:"fixed",bottom:0,left:0,right:0,height:56,background:"rgba(255,255,255,.92)",backdropFilter:"saturate(180%) blur(20px)",borderTop:".5px solid rgba(0,0,0,.08)",zIndex:100,justifyContent:"space-around",alignItems:"center",padding:"0 8px"}}>{[{id:"home",label:"Home",icon:"⌂"},{id:"claims",label:"Cases",icon:"📁"},{id:"chat",label:"Advisor",icon:"💬"}].map(t=>(<button key={t.id} onClick={()=>{if(t.id==="chat"){setActive(null);setMsgs([])}nav(t.id)}} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"none",border:"none",cursor:"pointer",padding:"4px 12px"}}><span style={{fontSize:18}}>{t.icon}</span><span style={{fontSize:10,fontWeight:600,color:view===t.id||(t.id==="chat"&&view==="chat")?"var(--blue)":"var(--g400)"}}>{t.label}</span></button>))}</div>
...
<div style={{marginBottom:8}}><div style={{fontSize:"clamp(20px, 4vw, 26px)",fontWeight:800,letterSpacing:-.8,marginBottom:4}}>{active.claimNumber}</div><div style={{fontSize:13,color:"var(--g500)",display:"flex",gap:10,flexWrap:"wrap"}}><span>👤 {active.worker}</span><span>🏢 {active.employer}</span><span>📅 {fmt(active.injuryDate)}</span><span>📋 {active.injuryType}</span>{active.injuryDate&&active.injuryDate!=="—"&&<span style={{color:"var(--blue)",fontWeight:500}}>Day {daysAgo(active.injuryDate)}</span>}</div></div>
...
{detailTab==="overview"&&<>
{/* RTW Progress */}
<RTWBar progress={getRTWProgress(active)}/>
{/* Deadlines */}
