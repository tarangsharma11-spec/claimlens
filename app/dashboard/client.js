...
`}</style>
<div style={{height:"100vh",display:"flex",flexDirection:"column"}} className={darkMode?"dark-mode":""}>

{/* NAV */}
<nav style={{height:56,padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fff",borderBottom:"1px solid #E8EAEF",flexShrink:0,zIndex:100}}>
  ...
  <div style={{display:"flex",gap:1,background:"#F0F1F3",borderRadius:10,padding:2}} className="desktop-nav">{[{id:"home",label:"Home"},{id:"claims",label:"My Cases"},{id:"advisor",label:"Advisor"},{id:"templates",label:"Templates"},{id:"analytics",label:"Analytics"},{id:"glossary",label:"Glossary"}].map(t=>(<button key={t.id} onClick={()=>{if(t.id==="advisor"){setActive(null);setMsgs([]);nav("chat")}else if(t.id==="templates")setShowTemplates(true);else if(t.id==="analytics")setShowAnalytics(true);else if(t.id==="glossary")setShowGlossary(true);else nav(t.id)}} style={{padding:"6px 14px",borderRadius:8,fontSize:12.5,fontWeight:600,color:(view===t.id||(t.id==="advisor"&&view==="chat"&&!active)||(t.id==="templates"&&showTemplates)||(t.id==="analytics"&&showAnalytics)||(t.id==="glossary"&&showGlossary))?"var(--g900)":"var(--g500)",background:(view===t.id||(t.id==="advisor"&&view==="chat"&&!active)||(t.id==="templates"&&showTemplates)||(t.id==="analytics"&&showAnalytics)||(t.id==="glossary"&&showGlossary))?"#fff":"transparent",border:"none",cursor:"pointer",boxShadow:(view===t.id||(t.id==="advisor"&&view==="chat"&&!active))?"0 1px 3px rgba(0,0,0,.04)":"none",letterSpacing:-.1}}>{t.label}</button>))}</div>
  <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}><button onClick={()=>setShowNotifs(!showNotifs)} style={{width:34,height:34,borderRadius:10,border:"1px solid #E8EAEF",background:showNotifs?"var(--g900)":"#fff",color:showNotifs?"#fff":"var(--g500)",cursor:"pointer",position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0"/></svg>{(()=>{const u=getNotifications(claims).filter(x=>x.type==="urgent").length;return u>0?<span style={{position:"absolute",top:-3,right:-3,width:16,height:16,borderRadius:"50%",background:"var(--red)",color:"#fff",fontSize:9,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>{u}</span>:null})()}</button><button onClick={()=>setQuickNote({open:true,text:"",caseId:""})} style={{width:34,height:34,borderRadius:10,border:"1px solid #E8EAEF",background:"#fff",color:"var(--g500)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} title="Quick Note (any case)"><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/></svg></button><button onClick={()=>setModal("new")} style={{padding:"7px 18px",borderRadius:10,fontSize:12.5,fontWeight:600,border:"none",background:"var(--blue)",color:"#fff",cursor:"pointer"}}>+ New Case</button><button onClick={()=>setDarkMode(!darkMode)} style={{width:34,height:34,borderRadius:10,border:"1px solid #E8EAEF",background:darkMode?"var(--g800)":"#fff",color:darkMode?"#fff":"var(--g500)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}} title="Toggle dark mode">{darkMode?"☀":"☾"}</button><button onClick={()=>signOut({callbackUrl:"/login"})} style={{padding:"7px 12px",borderRadius:10,fontSize:12,fontWeight:500,border:"1px solid #E8EAEF",background:"#fff",color:"var(--g500)",cursor:"pointer"}}>Sign Out</button></div>
</nav>
...
<div style={{padding:"20px 24px",background:"linear-gradient(135deg, #0071E3 0%, #00A6FB 50%, #34C759 100%)",borderRadius:20,marginBottom:20,color:"#fff",position:"relative",overflow:"hidden",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
...
return(<div style={{padding:"14px 16px",background:"#fff",borderRadius:14,border:"1px solid var(--g200)",boxShadow:"0 1px 3px rgba(0,0,0,.03)",marginBottom:14}}>
<div style={{fontSize:12,fontWeight:700,color:"var(--g400)",textTransform:"uppercase",letterSpacing:.8,marginBottom:8}}>Today's Agenda</div>
...
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(100px, 1fr))",gap:10,marginBottom:14}}>
  <div style={{padding:"14px",background:"#fff",borderRadius:12,border:"1px solid var(--g200)",boxShadow:"0 1px 3px rgba(0,0,0,.03)",textAlign:"center"}}><div style={{fontSize:26,fontWeight:800,letterSpacing:-1}}>{claims.length}</div><div style={{fontSize:10,fontWeight:500,color:"var(--g400)",marginTop:2}}>Total</div></div>
  {stageCounts.map(s=><div key={s.id} style={{padding:"14px",background:"#fff",borderRadius:12,border:"1px solid var(--g200)",boxShadow:"0 1px 3px rgba(0,0,0,.03)",textAlign:"center"}}><div style={{fontSize:26,fontWeight:800,letterSpacing:-1,color:s.color}}>{s.count}</div><div style={{fontSize:10,fontWeight:500,color:"var(--g400)",marginTop:2}}>{s.label}</div></div>)}
...
<div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:16,marginBottom:14}}>
  ...
  <div style={{overflow:"hidden",minWidth:0}}>
    ...
    {scenarios.map((s,i)=><button key={i} onClick={()=>{setActive(null);setMsgs([]);nav("chat");setTimeout(()=>send(s.t),100)}} style={{padding:"12px 14px",borderRadius:12,background:"#fff",border:"1px solid var(--g200)",boxShadow:"0 1px 3px rgba(0,0,0,.03)",cursor:"pointer",textAlign:"left",width:"100%",marginBottom:4,overflow:"hidden",maxWidth:"100%"}}><div style={{fontSize:13,fontWeight:600,marginBottom:1}}>{s.l}</div><div style={{fontSize:11,color:"var(--g500)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.t}</div></button>)}
  </div>
</div>
</>}
...
{/* Search bar */}
<div style={{marginBottom:12}}><input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="Search cases... (Cmd+K for quick switch)…" style={{width:"100%",padding:"10px 14px",borderRadius:12,border:".5px solid var(--g200)",fontSize:13,outline:"none",background:"#fff",fontFamily:"inherit"}} /></div>
...
<div style={{fontSize:12,fontWeight:700,color:"var(--g400)",textTransform:"uppercase",letterSpacing:1,marginBottom:12}}>Getting Started</div>
...
<div key={i} style={{padding:"18px",background:"#fff",borderRadius:14,border:"1px solid var(--g200)",boxShadow:"0 1px 3px rgba(0,0,0,.03)"}}>
...
<button onClick={()=>send("Can you explain that in simpler terms? Assume I have no legal background.")} style={{padding:"6px 14px",borderRadius:980,fontSize:11,fontWeight:500,border:"1px solid var(--g200)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Explain simply</button>
<button onClick={()=>send("What are my next steps based on this?")} style={{padding:"6px 14px",borderRadius:980,fontSize:11,fontWeight:500,border:"1px solid var(--g200)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>What are my next steps?</button>
<button onClick={()=>send("Can you summarize the key points in bullet form?")} style={{padding:"6px 14px",borderRadius:980,fontSize:11,fontWeight:500,border:"1px solid var(--g200)",background:"#fff",color:"var(--g600)",cursor:"pointer"}}>Summarize key points</button>
</div>}
{!active&&msgs.length===0&&<div style={{textAlign:"center",marginBottom:28,paddingTop:20}}>
<div style={{width:48,height:48,borderRadius:14,background:"var(--blue-light)",border:"1px solid var(--blue-border)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px"}}><svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="var(--blue)" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 01.865-.501 48.172 48.172 0 003.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z"/></svg></div>
...
{quickNote.open&&<div style={{position:"fixed",bottom:20,right:20,width:340,background:"#fff",borderRadius:16,boxShadow:"0 8px 32px rgba(0,0,0,.12)",border:"1px solid var(--g200)",zIndex:250,overflow:"hidden"}}>
...
<select value={quickNote.caseId} onChange={e=>setQuickNote(p=>({...p,caseId:e.target.value}))} style={{width:"100%",padding:"8px 10px",borderRadius:8,border:"1px solid var(--g200)",fontSize:12,outline:"none",background:"#fff",marginBottom:8}}>
<option value="">Select a case...</option>
{claims.map(c=><option key={c.id} value={c.id}>{c.claimNumber} - {c.worker}</option>)}
</select>
<textarea value={quickNote.text} onChange={e=>setQuickNote(p=>({...p,text:e.target.value}))} placeholder="Type your note..." rows={3} style={{width:"100%",padding:"8px 10px",borderRadius:8,border:"1px solid var(--g200)",fontSize:13,outline:"none",resize:"none",fontFamily:"inherit"}}/>
<button onClick={()=>{if(quickNote.caseId&&quickNote.text.trim()){setClaims(prev=>{const next=prev.map(c=>{if(c.id===quickNote.caseId){return{...c,timeline:[...(c.timeline||[]),{date:new Date().toISOString(),type:"note",note:quickNote.text.trim()}],notes:[...(c.notes||[]),{text:quickNote.text.trim(),date:new Date().toISOString()}]}}return c});persistClaims(user.email,next);if(active&&active.id===quickNote.caseId)setActive(next.find(c=>c.id===quickNote.caseId));return next});setQuickNote({open:false,text:"",caseId:""})}}} disabled={!quickNote.caseId||!quickNote.text.trim()} style={{width:"100%",marginTop:6,padding:"9px",borderRadius:10,border:"none",background:quickNote.caseId&&quickNote.text.trim()?"var(--blue)":"var(--g200)",color:"#fff",fontSize:13,fontWeight:600,cursor:quickNote.caseId&&quickNote.text.trim()?"pointer":"default"}}>Save Note</button>
</div>
</div>}

{/* TIMER */}
{timer.running&&<div style={{position:"fixed",bottom:quickNote.open?200:20,left:20,background:"#fff",borderRadius:12,boxShadow:"0 4px 20px rgba(0,0,0,.1)",border:"1px solid var(--g200)",zIndex:240,padding:"10px 16px",display:"flex",alignItems:"center",gap:12}}>
